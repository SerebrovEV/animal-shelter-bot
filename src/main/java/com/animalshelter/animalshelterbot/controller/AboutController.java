 ...
 * Запрос на информацию о приюте для собак через кнопку (callbackQuery) по значению {@link Callback#DOG_SHELTER_INFO}
 * Запрос на информацию о приюте для кошек через кнопку (callbackQuery) по значению {@link Callback#CAT_SHELTER_INFO}
 * Запрос на информацию об адресе и часах работы через кнопку (callbackQuery) по значению {@link Callback#DOG_SCHEDULE_INFO}
 * Запрос на информацию об адресе и часах работы через кнопку (callbackQuery) по значению {@link Callback#CAT_SCHEDULE_INFO}
 * Запрос на контактные данные для оформления пропуска приюта собак (callbackQuery) по значению {@link Callback#DOG_CAR_INFO}
 * Запрос на контактные данные для оформления пропуска приюта кошек (callbackQuery) по значению {@link Callback#CAT_CAR_INFO}
 */
@Component
...
public class AboutController implements CommandController {
    ...
    @com.animalshelter.animalshelterbot.handler.Callback(name = Callback.DOG_SHELTER_INFO)
    public SendMessage handleCallbackDogDescriptionMessage(CallbackQuery callbackQuery) {
        return new SendMessage(callbackQuery.from().id(), aboutDogDescriptionText)
         .replyMarkup(new InlineKeyboardMarkup()
                .addRow(new InlineKeyboardButton(backButtonText).callbackData(Callback.DOG_INFO_MENU.name())));

    }
    @com.animalshelter.animalshelterbot.handler.Callback(name = Callback.CAT_SHELTER_INFO)
    public SendMessage handleCallbackCatDescriptionMessage(CallbackQuery callbackQuery) {
        return new SendMessage(callbackQuery.from().id(), aboutCatDescriptionText)
         .replyMarkup(new InlineKeyboardMarkup()
                .addRow(new InlineKeyboardButton(backButtonText).callbackData(Callback.CAT_INFO_MENU.name())));

    }

    @com.animalshelter.animalshelterbot.handler.Callback(name = Callback.DOG_SCHEDULE_INFO)
    ...
    @com.animalshelter.animalshelterbot.handler.Callback(name = Callback.CAT_SCHEDULE_INFO)
    ...
    @com.animalshelter.animalshelterbot.handler.Callback(name = Callback.DOG_CAR_INFO)
    ...
    @com.animalshelter.animalshelterbot.handler.Callback(name = Callback.CAT_CAR_INFO)
