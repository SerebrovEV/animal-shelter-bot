...
public class StartController implements CommandController {
    private final String startText = "Привет! Данный бот может помочь вам взять и содержать животное из приюта. Для продолжения выберете команду:";

    @Command(name = "/start")
    public SendMessage handleStartMessage(Message message) {

        return new SendMessage(message.from().id(), this.startText)
